...
//Get the class selector of select element
const setTime = document.querySelector('.select');
//Use it to get the value from options
//using the eventlistener
//and reassinged the value of timer 
...
function settimer() {
  //ClearInterval to clear the previous time
  clearInterval(timeInterval);
   
 //variable timeChange is assign to the value of timer
 let timeChange = timer;
  const timer2 =  document.querySelector('.timer');
  //Use if condition to trigger the funtions inside
  if (timeChange > 0) {
    console.log(timeChange);
    
    //timeInterval value is reassigned to setInterval
    timeInterval = setInterval(() => {
      //where the class for display of timer is added
      timer2.classList.add("timerPhoto");
      //and display it to see
       document.querySelector('.timer').innerHTML = timeChange;
       //Use if statement 
       //where if the timer is equal to 0
       //it will clear the timeInterval which is this
       //part of the function to reset the timer
       //and remove the display 
       //and also capture the image
       if (timeChange === 0) {
        clearInterval(timeInterval);
        capturePhoto();
       ...
       //Time is dropping only until it
       //reaches 0 because the if statement condition
       // remove all this part after reaching the time 0
